...
cp -r * "${temp_dir}"
mkdir -p "${temp_dir}"/data/opt/flock

echo "Creating Forge package of Flock CLI from git rev $(git rev-parse --short HEAD)"

pushd .. > /dev/null
git archive HEAD | tar -x -C "${temp_dir}"/data/opt/flock
popd > /dev/null

pushd "${temp_dir}"/data/opt/flock > /dev/null
